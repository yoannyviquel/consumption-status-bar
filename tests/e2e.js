...
function run(config, d, { columns, statusCache, prStore, etaStore } = {}) {
  const home = homeWith(config);
  const claudeDir = path.join(home, '.claude');
  if (etaStore !== undefined) {
    fs.writeFileSync(path.join(claudeDir, 'statusline-eta.json'), JSON.stringify(etaStore), 'utf8');
  }
...
// --- eta element (task-estimate finish time) --------------------------------
const FLAG = String.fromCodePoint(0xf11e); // nf-fa-flag_checkered — the eta glyph
const MIN = 60 * 1000;

// The status line keys the estimate by the project (git repo root, else cwd).
// gitDir() returns a plain dir with a .git — path.resolve() matches projectKey().
const keyOf = (dir) => path.resolve(dir);

test('34. eta — no estimate set, segment hidden', () => {
  const g = gitDir('feature/x');
  const out = run(els('ctx', 'eta'), data({ cwd: g, ctx: 20 })); // no etaStore
...
test('35. eta — under estimate shows the finish clock', () => {
  const g = gitDir('feature/x');
  // started 5 min ago, 45 min estimate -> ~40 min remaining -> a future clock time.
  const out = run(els('eta'), data({ cwd: g, ctx: 20 }), {
    etaStore: { [keyOf(g)]: { startMs: Date.now() - 5 * MIN, minutes: 45 } },
  });
  startsAndEndsCapped(out);
  assert.ok(strip(out).includes(FLAG), 'shows the eta glyph');
  assert.ok(/\d+h\d\d/.test(strip(out)), `shows a HHhMM clock, got ${JSON.stringify(strip(out))}`);
});

test('36. eta — overdue shows an overrun (+…)', () => {
  const g = gitDir('feature/x');
  // started 90 min ago, 30 min estimate -> 60 min overdue.
  const out = run(els('eta'), data({ cwd: g, ctx: 20 }), {
    etaStore: { [keyOf(g)]: { startMs: Date.now() - 90 * MIN, minutes: 30 } },
  });
  startsAndEndsCapped(out);
  assert.ok(strip(out).includes(FLAG), 'shows the eta glyph');
...
test('37. eta — stale estimate (>24h) is ignored', () => {
  const g = gitDir('feature/x');
  const out = run(els('ctx', 'eta'), data({ cwd: g, ctx: 20 }), {
    etaStore: { [keyOf(g)]: { startMs: Date.now() - 48 * 60 * MIN, minutes: 30 } },
  });
  assert.ok(!strip(out).includes(FLAG), 'a day-old un-cleared estimate is dropped');
});

test('38. eta — estimate keyed to another project is ignored', () => {
  const g = gitDir('feature/x');
  const out = run(els('ctx', 'eta'), data({ cwd: g, ctx: 20 }), {
    etaStore: { 'C:\\some\\other\\project': { startMs: Date.now(), minutes: 30 } },
  });
