...
//   eta           : clock time the current task's estimate lands on. Claude
//                   records an estimate (start + minutes) for the current project
//                   before coding (set-eta.js / /statusline-eta); this shows
//                   start + minutes as a clock, green→red as it's consumed, then
//                   `+Xm` once overdue. Project-scoped, not tied to a ticket.
//                   Shown only while an (un-cleared, <24h) estimate exists.
...
// --- eta: estimated finish time of the current task -------------------------
// "At what clock time does the current task's estimate land?" Claude records an
// estimate (start + minutes) for the current project before it starts coding
// (via set-eta.js / /statusline-eta); this element shows start + minutes as a
// clock, turning green→red as the estimate is consumed, then `+Xm` once overdue.
// Project-scoped (one estimate in flight per repo/dir), not tied to any ticket.
// Best-effort: any failure or missing/stale estimate drops the segment (null).

const ETA_PATH = () => path.join(os.homedir(), '.claude', 'statusline-eta.json');
const ETA_MAX_AGE = 24 * 60 * 60 * 1000; // ignore an estimate left un-cleared for over a day

function readJson(p) {
  try { return JSON.parse(fs.readFileSync(p, 'utf8')) || null; } catch { return null; }
}

// Store key for the current work: the git repo root if any, else the directory.
// Writer (set-eta.js) computes the same key from its cwd, so both agree.
function projectKey(dir) {
  return path.resolve(gitRoot(dir) || String(dir || process.cwd()));
}
...
function etaSegment(d) {
  const cwd = d.workspace?.current_dir || d.cwd || process.cwd();
  const store = readJson(ETA_PATH());
  const e = store && store[projectKey(cwd)];
  const startMs = Number(e && e.startMs);
  const minutes = Number(e && e.minutes);
  if (!(startMs > 0) || !(minutes > 0)) return null;
  if (Date.now() - startMs > ETA_MAX_AGE) return null; // stale — estimate never cleared

  const estMs = minutes * 60000;
  const endMs = startMs + estMs;
  const remaining = endMs - Date.now();
  // Green at the start, red as the estimate is consumed / exceeded.
  const bg = grad(Math.min(1, Math.max(0, (Date.now() - startMs) / estMs)));
  const label = remaining > 0 ? fmtClock(endMs) : `+${fmtDurationShort(-remaining)}`;
